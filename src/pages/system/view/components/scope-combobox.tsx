...
export function ScopeCombobox({ value, onChange, disabled }: ScopeComboboxProps) {
  ...
  const currentLabel = scopeOptions.find((option) => option.value === value)?.label || value;
    ...
    <Popover open={open && !disabled} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role='combobox'
          aria-expanded={open}
          className={cn("w-full justify-between", disabled && "opacity-50 cursor-not-allowed text-muted-foreground")}
          disabled={disabled}
        >
      ...
      <PopoverContent className='w-[--radix-popover-trigger-width] p-0'>
        <Command filter={(value, search) => (value.includes(search) ? 1 : 0)}>
          <CommandInput placeholder='Search or create scope...' value={inputValue} onValueChange={setInputValue} />
          <CommandList>
            <CommandEmpty>
